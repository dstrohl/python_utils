...
from PythonUtils.ConverterHelper.converter_helper import ConverterHelper, NoConverterError, ConversionError, \
...
class TestConverter(TestCase):
    ...
    def test_via(self):
        test_sets = [
            (0, 'int', 1, 'float', 'str', '1.0'),
            (1, 'yesno', 'Yes', 'bool', 'str', 'True'),
        ]

        for ts in test_sets:
            tmp_name = '#%s: From(%s): %s => via(%s) => To(%s): %s' % (ts[0], ts[1], ts[2], ts[3], ts[4], ts[5])
            with self.subTest(tmp_name):
                tmp_ret = self.ch(ts[2], fr=ts[1], to=ts[4], via=ts[3])
                # print(tmp_name)
